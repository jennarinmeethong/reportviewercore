...
namespace Microsoft.Reporting.WinForms
{
	...
	public sealed class LocalReport : Report, ISerializable, IDisposable
	{
		...
		private static IEnumerable ToPortableDataSet(object value)
		{
			...
			if (enumerable != null && !(value is Type))
			{
				return enumerable;
			}
		...
		{
			switch (format == null ? string.Empty : format.Trim().ToUpperInvariant())
			{
				case "PDF": return PortableFormat.Pdf;
				case "HTML":
				case "HTML5": return PortableFormat.Html;
				case "EXCELOPENXML": return PortableFormat.ExcelOpenXml;
				case "WORDOPENXML": return PortableFormat.WordOpenXml;
				default: throw new ArgumentOutOfRangeException(nameof(format), format, "The v2 portable bridge supports PDF, HTML, EXCELOPENXML, and WORDOPENXML.");
			}
		}
