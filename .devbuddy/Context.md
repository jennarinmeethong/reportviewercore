...
The test suite now also covers allow-listed `Len`/`Trim`/`UCase`/`LCase`/`InStr`/`Replace`, case-insensitive dataset lookup, OpenXML internal relationship targets, worksheet dimensions, report-sized DOCX sections, native Word multiline breaks, paginated Word/Excel outputs, invalid legacy page-source inputs, and HTTP transport endpoint/format rejection.
...
The fixture coverage index lives in `tests/fixtures/README.md`; source and test-output RDLC filenames must match exactly, and the current set contains 38 fixtures. The empty-tablix `NoRowsMessage` path is covered by `no-rows-message.rdlc` across HTML/PDF/XLSX/DOCX, pure search/replace functions by `string-functions-advanced.rdlc`, and native HTML/Word multiline breaks by `multiline.rdlc`.
