...
The test suite now also covers allow-listed `Len`/`Trim`/`UCase`/`LCase`/`InStr`/`Replace`/`Mid`, quoted-literal-safe comparisons, case-insensitive dataset/parameter lookup, OpenXML internal relationship targets, worksheet dimensions, report-sized DOCX sections, native Word multiline breaks, paginated Word/Excel outputs, invalid legacy page-source inputs, HTTP transport endpoint/format rejection, nested page decorations, mixed tablix/subreport composition, explicit unsupported nested subreport diagnostics, recursive tablix-cell visual offsets, shared page-size/delegate validation, finite chart values, network-path hyperlink rejection, finite font/page-size validation, and vertical PDF hyperlink annotations.
...
The fixture coverage index lives in `tests/fixtures/README.md`; source and test-output RDLC filenames must match exactly, and the current set contains 43 fixtures. The empty-tablix `NoRowsMessage` path is covered by `no-rows-message.rdlc` across HTML/PDF/XLSX/DOCX, pure search/replace functions by `string-functions-advanced.rdlc`, native HTML/Word multiline breaks by `multiline.rdlc`, nested page decorations by `nested-header-footer.rdlc`, mixed top-level tablix/subreport content by `mixed-tablix-subreport.rdlc`, and explicit unsupported nested subreports by the two `unsupported-*-subreport.rdlc` fixtures.
