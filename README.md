...
The v2 local workflow is `ReportViewerCore.Headless.LocalReport`; it accepts an RDLC definition, enumerable data sets, parameters, explicit image/subreport resolvers, and injected renderers. The portable engine currently covers constrained multi-tablix, grouping, subreport, pagination, expressions, and mixed textbox/image/rectangle/line/basic-bar-chart paths, including those visual items inside tablix cells. Remote reports use `ServerReport`; `HttpReportServerTransport` supports URL access while `IReportServerAuthenticator` keeps credentials explicit. Basic bar charts produce native XLSX/DOCX chart parts.
