# ReportViewer Core
This project is a port of Microsoft Reporting Services (Report Viewer) to .NET 6+. The legacy compatibility packages target existing applications; the cross-platform v2 packages are preview software and are not full SSRS/RPL parity. This project is not officially supported by Microsoft.

## Cross-platform v2 migration

The repository is incrementally introducing a backend-neutral rendering layer for headless PDF, HTML, and OpenXML rendering on macOS Apple Silicon, Linux, and Windows. The first slice is available in `ReportViewerCore.Rendering.Abstractions`, `ReportViewerCore.Rendering.Skia`, `ReportViewerCore.Rendering.Windows`, `ReportViewerCore.Rendering.Html`, `ReportViewerCore.Rendering.OpenXml`, `ReportViewerCore.Engine`, and `ReportViewerCore.Headless`. See the [v2 status](docs/cross-platform-v2.md) and [migration checklist](docs/cross-platform-v2-checklist.md) before selecting it for a production workflow.

Run the headless smoke sample with `dotnet run --project ReportViewerCore.Sample.CrossPlatform -- ./artifacts/cross-platform`. On Windows, `ReportViewerCore.Sample.WinForms.V2` demonstrates the WinForms display adapter over the same SkiaSharp-rendered pages. After packing v2 libraries, use `python3 .devbuddy/tools/validate_v2_artifacts.py` to validate package and smoke artifacts. The existing `Microsoft.ReportViewer.NETCore` and `Microsoft.ReportViewer.WinForms` packages remain legacy compatibility lines; use the v2 projects for the new backend-neutral workflow.

The v2 local workflow is `ReportViewerCore.Headless.LocalReport`; it accepts an RDLC definition, enumerable data sets, parameters, explicit image/subreport resolvers, and injected renderers. The portable engine currently covers constrained multi-tablix, grouping, subreport, pagination, expressions including culture-aware `FormatNumber(value, digits)`, bounded `Left`/`Right`, and fixed-arity `CStr`, plus mixed textbox/image/rectangle/line/basic-bar-chart paths, including those visual items inside tablix cells. Remote reports use `ServerReport`; `HttpReportServerTransport` supports URL access while `IReportServerAuthenticator` keeps credentials explicit. Basic bar charts produce native XLSX/DOCX chart parts.

The v2 packages are preview software, not a full SSRS/RPL replacement. Advanced recursive tablix member geometry, interactive toggles, richer chart/map contracts, printer/legacy formats, and matching Windows/RID runtime validation remain outside the local portable gate. Do not load or execute RDLC files from untrusted sources; the constrained expression host intentionally does not provide arbitrary report code execution.
